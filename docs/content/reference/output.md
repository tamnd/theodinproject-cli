...
describes what users get. Pick a format with `-o`, or let theodinproject choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
theodinproject <command> -o table   # aligned columns for reading
theodinproject <command> -o jsonl   # one JSON object per line, for piping
theodinproject <command> -o json    # a single JSON array
theodinproject <command> -o csv     # spreadsheet friendly
theodinproject <command> -o tsv     # tab-separated
theodinproject <command> -o url     # just the URL column
theodinproject <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
theodinproject <command> --fields id,title,url
```
...
```bash
theodinproject <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
theodinproject <command>            # a table, because this is a terminal
theodinproject <command> | wc -l    # JSONL, because this is a pipe
```
